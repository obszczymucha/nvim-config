...
function M.get_project_root_dir( dir )
  ...
  return vim.fs.root( 0, { ".luarc.json", ".git" } ) or vim.fn.getcwd()
end
...
---@param hex_color string Hex color code, e.g. "#ff5733"
---@param hue_shift number Hue shift in degrees (-360 to 360)
---@return string Hex color code with adjusted hue
function M.hue( hex_color, hue_shift )
  -- Remove # if present
  hex_color = hex_color:gsub( "#", "" )
